...
```{r remove, echo=FALSE, include=FALSE}
rm(BobRossEpi, SmallBobRoss, BobRoss)
ls()
```

```{r create-BobRossStep10}
...
```{r create-BobRossStep11}
# ls()
BobRossStep11 <- BobRossStep10 %>% 
  # reorganize columns
  dplyr::select(season, episode, title, object, present) %>% 
...
dplyr::mutate(.data = BobRossStep11, 
              title_category = case_when(
                ...
                str_detect(string = title, pattern = "Winter") ~ "Season", 
                str_detect(string = title, pattern = "Sunset") ~ "Sunsets",
                str_detect(string = title, pattern = "River") ~ "Body of Water",
                str_detect(string = title, pattern = "Cabin") ~ "Cabins",
                str_detect(string = title, pattern = "Woods") ~ "Woods",
                str_detect(string = title, pattern = "Lake") ~ "Body of Water",
                str_detect(string = title, pattern = "Stream") ~ "Body of Water",
                str_detect(string = title, pattern = "Snow") ~ "Snow",
                str_detect(string = title, pattern = "Valley") ~ "Valleys",
                str_detect(string = title, pattern = "Seascape") ~ "Seascapes"))
```

Create title/season 1 categories.

```{r}
dplyr::mutate(.data = BobRossStep11, 
              title_season_category = case_when(
                # season 1 titles with mountains
                season == 1 & str_detect(title, "MOUNTAINS|MT") ~ "S01 Mountains",
                season == 1 & str_detect(title, "WOODS") ~ "S02 Woods", 
                season == 1 & str_detect(title, "WINTER") ~ "S01 Winter",
                season == 1 & str_detect(title, "SUNSET") ~ "S01 Sunset"))
```
