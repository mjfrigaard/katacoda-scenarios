...
```{r}
stringr::str_view_all(string = unique(BobRossStep11$object), 
                      pattern = "_frame", 
                      match = TRUE)
```
```{r}
dput(unique(BobRossStep11$title))
```


```{r other-objects}
count(BobRossStep11, episode)
```
...
BobRossStep11 %>% 
  dplyr::mutate(
    episode_category = case_when(
            str_detect(string = title, pattern = "WOODS") ~ "Wood paintings",
              str_detect(string = title, pattern = "MOUNTAINS") ~ "mountain paintings"))
```
