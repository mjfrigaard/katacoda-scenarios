...
```{r}
unique(BobRossLong$object)
BobRossLong
```

```{r}
dplyr::mutate(.data = BobRossStep11, 
              season01_objects = case_when(season == 1 & present == 1 ~ "present in season 1",
                                           season == 1 & present == 0 ~ "absent in season 1")) %>% 
  dplyr::count(title, season01_objects)
```
