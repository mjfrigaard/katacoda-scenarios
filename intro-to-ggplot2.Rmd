...
# fs::dir_ls("data")
Brexit <- readr::read_csv("data/economist-brexit.csv")
...
We're going to continue building our plot by adding the `ggplot2::geom_point()` function, and we'll also include the `show.legend = FALSE` argument again to remove the legend for the two `poll` categories.
...
`ggplot2` allows us to build graphs layer-by-layer, using the geoms and aesthetics to create plots that are clear and accurate. Each time we need to add something to a graph, we can either add a new `geom`, or look for ways to adjust the geoms with new aesthetic options. 
