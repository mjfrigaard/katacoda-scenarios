...
```{r gg_step6_bar_01-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step6_bar_01-1.png")
```
...
```{r gg_step6_col_02-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step6_col_02-1.png")
```
...
```{r gg_step7_hist_02-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step7_hist_02-1.png")
```
...
```{r gg_step6_density_02-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step6_density_02-1.png")
```
...
```{r gg_step7_ridge_01-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step7_ridge_01-1.png")
```
...
```{r gg_step8_boxplot_01-1, echo=FALSE}
knitr::include_graphics(path = "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/gg_step8_boxplot_01-1.png")
```
...
```{r export-comic_characters.csv, include=FALSE, eval=TRUE}
fivethirtyeightdata::comic_characters %>% 
  write_csv(x = ., file = "../data/comic_characters.csv")
```

```{r ComicData, message=FALSE, warning=FALSE}
# click to execute code
...
```{r hickey-feature-comics-3-2, echo=FALSE}
knitr::include_graphics(path = "https://fivethirtyeight.com/wp-content/uploads/2014/10/hickey-feature-comics-3.png?w=1220")
```
...
Up to this point, the data we've presented resulted in a single visualization. Sometimes data are so complex and layered that they will require multiple graphs. In this case, we will want to arrange multiple graphs together in a grid, or facet. Graphs presented this way are called ['Small Multiples'](https://en.wikipedia.org/wiki/Small_multiple), a term popularized by [Edward Tufte](https://www.edwardtufte.com/tufte/).
...
```{r ImdbData, message=FALSE, warning=FALSE}
# click to execute code
...
The number of unique responses (`n_unique`) for each character variable is a good source for sanity checks. For example, the largest number of unique responses belongs to the title id variable (`tconst` = `136925`), and this is identical to the number of rows in the dataset. The next largest number belongs to the `originalTitle` variable (`128171`), and the documentation tells us this variable is the title for the film in it's original language. By itself, this number doesn't tell us much, but we can see the next largest number (`124228`) is the film's `primaryTitle`, and it makes sense that the number of unique responses for these two variables are almost the same. 

It also makes sense that the unique identifier for actor/actress (`nconst`) is close to the actor/actress `primaryName`. There should be way more titles (`originalTitle` or `primaryTitle`) than `genres`, and there is (`1056`).

Finally, we can see the two binary variables we read about above (`category` and `isAdult`) only list `2` unique values (in `n_unique`), so we can feel confident these were imported correctly.
...
+ The average movie rating is a `6.00`, which also makes sense considering it's a 1-10 scale. 

+ The number of votes (`numVotes`) is the most skewed variable because it ranges from `10` to `2334927`. 

+ The `startYear` for the movie has an average of `1985`, and increases steadily from `1906` to `2021`, which makes sense because more movies are being made every year. 
...
The average age of the actor/actress is `38.4`, with a low of `1` and a high of `98` (both plausible).

### Number of votes vs. average rating

We've been asked to help explain the relationship between the average rating a movie received (`averageRating`), and the number of votes that went into the score (`numVotes`). 
...
We will look to see how much the `averageRating` for movies has increased over the decades. In order to do this, we need a categorical variable from the `startYear` variable. The `cut()` function is handy for this because we can supply the number of `breaks` we want to split the numeric `startYear` variable into (`12` in this case). We will also create some clear labels for this categorical variable with the `labels` argument, and make sure the format is `ordered`.
...
### Scatterplots (`geom_jitter`)

Now we want to make sure 

```{r labs_jitter_avgratng_yearcat10}
labs_jitter_avgratng_yearcat10 <- labs(
...
```{r}
ImdbData %>% 
  ggplot(aes(x = year_cat10, y = numVotes)) + 
  geom_jitter(alpha = 2/5, show.legend = FALSE) + 
  labs_jitter_avgratng_yearcat10
```



### Violin plots (`geom_jitter`)
...
```{r gg_step17_ridge__01}
# # click to execute code
gg_step17_ridge__01 <- ImdbData %>%
  ...
  guides(fill = FALSE) +
  # add labels
  labs_ridge_avg_ratng_year_cat10
# # save
# ggsave(plot = gg_step9_ridge_01,
#        filename = "gg-step9-ridge-01.png",
#        device = "png",
...
If you wish to facilitate direct comparisons you ideally need to juxtapose individual frames within the same view. The common technique used to achieve this in visualization is through small multiples, where you repeat the same representation for each moment in time of interest and present them collectively in an adjacent view, often through a grid layout. This enables far more incisive comparisons.
...
```{r WW2Aces, message=FALSE, warning=FALSE}
WW2Aces <- readr::read_csv("https://bit.ly/38QNLjU")
WW2Aces %>% glimpse()
```


```{r WW2Aces-filter-air_vic}
WW2Aces %>% 
  filter(air_vic > 10)
```

```{r TopWW2AcePerCountry}
TopWW2AcePerCountry <- WW2Aces %>% 
  # all aces
  filter(air_vic >= 10) %>% 
  # 
  group_by(country) %>% 
  top_n(n = 1, wt = air_vic) %>% 
  mutate(country = reorder(country, air_vic)) %>% 
  
TopWW2AcePerCountry
```

```{r TopWW2AcePerCountry-geom_point}
TopWW2AcePerCountry %>% 
  ggplot(aes(x = air_vic, 
             y = forcats::fct_reorder(country, air_vic), 
             fill = country, 
             group = force)) + 
  geom_point(show.legend = FALSE) 
```

```{r TopWW2AcePerCountry-geom_point-facet_wrap}
TopWW2AcePerCountry %>% 
  ggplot(aes(x = air_vic, y = country, fill = country, group = force)) + 
  geom_point(show.legend = FALSE) + 
  facet_wrap(. ~ force)
```
