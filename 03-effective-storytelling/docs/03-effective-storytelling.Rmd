...
The number of unique (`n_unique`) of dates prints to the next line. This is because the terminal window has a width limit, so wide content is printed to the line below.
...
# "https://raw.githubusercontent.com/mjfrigaard/katacoda-scenarios/master/03-effective-storytelling/docs/img/n_unique-col.png"
knitr::include_graphics("img/n_unique-col.png")
```
...
Below we use `skimr::focus()` and `skimr::yank()` to view the mean, standard deviation, minimum, maximum, and a small histogram for the numeric variables in the `People` dataset:
...
In this case, we have two variables in this output: `birthCountry` and `n`. If we're going to build a graph from these data, we know we'll need a way to represent both the name of the country, and the number of times it occurs.
...
If we find the `x` axis is difficult to read, we can pivot the display of the columns with the `ggplot2::coord_flip()` function. Note that we also need to change the `forcats` function in `mutate()` to `fct_reorder()`.
...
We will need to open the `gg-step6-bar-02.png` graph in the vscode IDE (above the Terminal console). 

### Communication tips

Bar charts and column charts display the counts for each different response in categorical variable. We can help our audience interpret these graphs by always setting the count axis scale to zero, sorting the responses in the chart so make it easier to read, and flipping the `x` axis if it is difficult to read.  
...
```{r gg_step8_boxplot_01, message=FALSE, warning=FALSE}
...
### Multiple variable distributions (4)
