...
Launch an R console by clicking here -> `R` (Click on the *Run command* icon)
...
```{r step3-package-no-show, message=FALSE, warning=FALSE, echo=FALSE}
# install.packages("baseballDBR")
library(baseballDBR)
```

```{r step3-package, message=FALSE, warning=FALSE, eval=FALSE}
install.packages("baseballDBR")
library(baseballDBR)
```
...
We are going to load a dataset which contains information about password quality & strength. Read more about the passwords data [here](hhttps://docs.google.com/spreadsheets/d/1cz7TDhm0ebVpySqbTvrHrD3WpxeyE4hLZtifWSnoNTQ/edit#gid=5).

```{r , message=FALSE, warning=FALSE}
PWords <- readr::read_csv('https://bit.ly/2JXiMck')
PWordsSkim <- skimr::skim(PWords)
```

### Review data 

View the character variables in `PWordsSkim`
...
View the numeric variables in `PWordsSkim`
...
*Do these data make sense after reading the documentation?*
