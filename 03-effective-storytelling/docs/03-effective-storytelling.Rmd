...
Data with extreme values like this can be removed or transformed. If we have a reason to exclude these data, we can do `filter` the data to only the range we want in the graph (say `Number of votes < 1500000`), and then include this in the graph. 

Run the code below to see what this would look like:

```{r gg_step18_jitter_02}
# click to execute code
...
Another option is to transform the axis, which we can do with `ggplot2::scale_y_log10()`. This changes the `y` axis scale using a base-10 log transformation.

We will also use the handy [`label_log10` function](https://bit.ly/35Ywt2q) developed by [Claus Wilke](https://wilkelab.org/)

```{r gg_step18_jitter_03}
# click to execute code
...
Open the `gg-step18-jitter-03.png` graph in the vscode IDE (above the Terminal console). We can see the `log10` transformation spreads the points out more uniformly across the `y` axis, but is also makes it more difficult to interpret the number of votes. 

Regardless of choice we make (removal of extreme values or transforming the scale), we want to make sure we communicate either of these changes to our stakeholders' and include them in any reports or write-ups so we don't give them a distorted view of the underlying variable.
...
```{r skim-averageRating}
ImdbData$averageRating %>% 
  skimr::skim()
```

The distribution for `averageRating` looks evenly distributed around the mean and median, and the `hist` looks fairly symmetrical. We will see if this holds true for `averageRating` when we look at it across the levels of `year_cat10`.
...
Open the `gg-step19-violin-01.png` graph in the vscode IDE (above the Terminal console). We can see the violin plot shows us the `averageRating` variable as a smoothed area, with tails extending out to the ends of the distribution (towards the 'neck' of the violin).

In the next section, we are going to examine how the `numVotes` and `averageRating` relate to each other over time. 
...
This enables far more incisive comparisons.

`facet_wrap`

```{r gg_step20_facet_02}
