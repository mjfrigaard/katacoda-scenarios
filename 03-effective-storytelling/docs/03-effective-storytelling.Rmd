...
Box plots display many of the numbers we see in the `skimr` output. Click on the code below to create a `skimr` summary on the `stars` ratings for the Ramen in `Tray`s.
...
The figure below displays a box-plot for the distribution of `stars` ratings across `Tray` level of `style`. The summary statistics from the `skimr` output have been labeled on the graph. The 25th percentile (or `p25`) is the first vertical line of the box (or hinge). The 50th percentile (or `p50`) is the median and middle line of the box. The 75th percentile (`p75`) is the last vertical line in the box (or hinge). 
...
We've also labeled the minimum (`p0`) and maximum (`p100`) values, and the interquartile range (which is similar to the standard deviation). We can calculate the interquartile range using `dplyr` below:
