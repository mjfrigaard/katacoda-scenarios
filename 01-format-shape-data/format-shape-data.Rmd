...
```{r step8-unite-episode_new, eval=FALSE}
# click to copy code
...
```{r step8-unite-output, eval=FALSE}
# A tibble: 27,001 x 4
...
```{r step9-mutate-template, eval=FALSE}
...
```{r step11-case_when-template, eval=FALSE}
...
```{r step11-case_when-object_category}
...
```{r step11-object_category-condition-2}
dplyr::mutate(.data = BobRossStep11,
          object_category = case_when(
              season == 1 & str_detect(object, "mountain") ~ "mountains",
              season == 1 & str_detect(object, "deciduous") ~ "trees",
...
```{r step11-object_category-case_when-catchall}
...
```{r step12-pivot_longer, eval=FALSE}
...
```{r step12-pivot_longer-assign, include=FALSE}
BobRossStep12 <- tidyr::pivot_longer(data = BobRossStep12, 
                                     cols = -c(episode, season, episode_num, title), 
                                     names_to = "object",  
                                     values_to = "present")
...
```{r step12-painting_cats, eval=FALSE}
...
```{r step12-values_from, eval=FALSE}
